...
contract CrowdfundingTest is Test {
    ...
    function setUp() external {
        ...
        fundraising_Campaign = new Fundraising_Campaign(
            address(fundRaiserRegisteration),
            address(funderRegisteration)
        );
    }
    ...
    function testFundraiserRegisteration() external {
        fundRaiserRegisteration.register(
            "John Doe",
            "w8M9d@example.com",
            "United States"
        );
        assertEq(
            fundRaiserRegisteration.IsRegisteredFundRaiser(address(this)),
            true
        );
    }
    ...
    function testgetFundraiserDetailsByAddress() external {
        ...
        ) = fundRaiserRegisteration.getFundraiserDetailsByAddress(
                address(this)
            );
        assertEq(name, "John Doe");
        assertEq(email, "w8M9d@example.com");
    ...
    function testSubmitCampaignAndGetCampaigndetails() external {
        ...
        vm.stopPrank();
        bool isRegistered = fundRaiserRegisteration.IsRegisteredFundRaiser(
            Owner
        );
        console2.log("Registered?", isRegistered);
        ...
        assertEq(
            fundraising_Campaign.getCampaignDetails(campaignId).title,
            "Test Campaign"
        );
        assertEq(
            fundraising_Campaign.getCampaignDetails(campaignId).description,
            "This is a test campaign"
        );
        assertEq(
            fundraising_Campaign.getCampaignDetails(campaignId).goalAmount,
            100 ether
        );
        assertEq(
            fundraising_Campaign.getCampaignDetails(campaignId).deadline,
            block.timestamp + 1 weeks
        );
        assertEq(
            fundraising_Campaign.getCampaignDetails(campaignId).category,
            "Charity"
        );
    }
}
